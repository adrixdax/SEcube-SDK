...
uint16_t se3_algo_Shake128_update(
    ...
    *dataout_len = 0;
    ...
    if (flags & SE3_CRYPTO_FLAG_FINIT) {
        if (SHAKE_RES_OK != shake128_finalize(&state->keccak)) {
            return SE3_ERR_HW;
        }
        if (SHAKE_RES_OK != shake128_squeeze(dataout, state->output_len, &state->keccak)) {
            return SE3_ERR_HW;
        }
...
uint16_t se3_algo_Shake256_init(se3_flash_key* key, uint16_t mode, uint8_t* ctx) {
    se3_shake_ctx* state = (se3_shake_ctx*)ctx;
    state->output_len = mode;
    if (SHAKE_RES_OK != shake256_init(&state->keccak)) {
        return SE3_ERR_PARAMS;
    }
    return SE3_OK;
}
...
uint16_t se3_algo_Shake256_update(
    ...
    *dataout_len = 0;
    ...
    if (datain1_len > 0) {
        if (SHAKE_RES_OK != shake256_absorb(&state->keccak, datain1, datain1_len)) {
            return SE3_ERR_HW;
        }
    }

    if (flags & SE3_CRYPTO_FLAG_FINIT) {
        if (SHAKE_RES_OK != shake256_finalize(&state->keccak)) {
            return SE3_ERR_HW;
        }
        if (SHAKE_RES_OK != shake256_squeeze(dataout, state->output_len, &state->keccak)) {
            return SE3_ERR_HW;
        }
        *dataout_len = state->output_len;
    }
